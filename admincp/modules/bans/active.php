...
try {
	...
	$banType = check($_GET['type']) ? $_GET['type'] : 'permanent';
	
	$BanSystem = new AccountBan();
	$BanSystem->setBanType($banType);
	$banList = $BanSystem->getActiveBanList();
	
	echo '<div class="row">';
		echo '<div class="col-sm-12 col-md-12 col-lg-8">';
			echo '<ul class="nav nav-tabs">';
				echo '<li role="presentation" '.($banType == 'permanent' ? 'class="active"' : null).'><a href="'.admincp_base('bans/active/type/permanent').'">Permanent Bans</a></li>';
				echo '<li role="presentation" '.($banType == 'temporal' ? 'class="active"' : null).'><a href="'.admincp_base('bans/active/type/temporal').'">Temporal Bans</a></li>';
			echo '</ul>';
		echo '</div>';
	echo '</div>';

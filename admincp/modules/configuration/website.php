...
if(check($_POST['settings_submit'])) {
	try {
		...
		// server files
		if(!check($_POST['server_files'])) throw new Exception('Invalid Server Files setting.');
		if(!in_array($_POST['server_files'], array('IGCN', 'CUSTOM'))) throw new Exception('Invalid Server Files setting.');
		...
		// guild mark bin2hex
		if(!check($_POST['gmark_bin2hex_enable'])) throw new Exception('Invalid Guild Logo setting.');
					...
					echo '<table class="table table-striped table-bordered table-hover" style="table-layout: fixed;">';
						...
						echo '<tr>';
							...
							echo '<td>';
								echo '<div class="radio">';
									echo '<label>';
										echo '<input type="radio" name="server_files" value="IGCN" '.(config('server_files') == 'IGCN' ? 'checked' : null).'>';
										echo 'IGCN';
									echo '</label>';
								echo '</div>';
								echo '<div class="radio">';
									echo '<label>';
										echo '<input type="radio" name="server_files" value="CUSTOM" '.(config('server_files') == 'CUSTOM' ? 'checked' : null).'>';
										echo 'CUSTOM';
									echo '</label>';
								echo '</div>';
							echo '</td>';
						...
						echo '<tr>';
							...
							echo '<td>';
								echo '<div class="radio">';
									echo '<label>';
										echo '<input type="radio" name="gmark_bin2hex_enable" value="1" '.(config('gmark_bin2hex_enable') ? 'checked' : null).'>';
										echo 'Enabled';
									echo '</label>';
								echo '</div>';
								echo '<div class="radio">';
									echo '<label>';
										echo '<input type="radio" name="gmark_bin2hex_enable" value="0" '.(!config('gmark_bin2hex_enable') ? 'checked' : null).'>';
										echo 'Disabled';
									echo '</label>';
								echo '</div>';
							echo '</td>';
						...
						echo '<tr>';
							echo '<td>';
								...
								echo '<p class="setting-description">If enabled, you will be able to use thrid-party websites to manage the execution of your cron jobs.<br /><br />By default, your cron jobs api is located at:<br /><span style="color:red;">'.dirname(__BASE_URL__).'/api/cron.php</span></p>';
							echo '</td>';
							echo '<td>';
								echo '<div class="radio">';
									echo '<label>';
						...
						echo '<tr>';
							echo '<td>';
								...
								echo '<p class="setting-description">Private key to secure your cron jobs api from public execution.<br /><br />Used by sending the key through a <strong>$_GET</strong> variable:<br /><span style="color:red;">'.dirname(__BASE_URL__).'/api/cron.php?key=private_key</span></p>';
							echo '</td>';
							echo '<td>';

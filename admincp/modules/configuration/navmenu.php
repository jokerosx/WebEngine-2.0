...
try {
	...
	if(check($_POST['navbar_submit'])) {
		try {
			# cfg
			$newCfg = loadConfig('navbar');
			if(!is_array($newCfg)) throw new Exception('Navbar configs empty.');
			
			if(!check($_POST['navbar_id'])) throw new Exception('Please fill all the form fields.');
			if(!check($_POST['navbar_type'])) throw new Exception('Please fill all the form fields.');
			if(!check($_POST['navbar_phrase'])) throw new Exception('Please fill all the form fields.');
			...
			$newElementData = array(
				...
				'link' => (check($_POST['navbar_link']) ? $_POST['navbar_link'] : ''),
	...
	echo '<div class="row">';
		echo '<div class="col-sm-12">';
			echo '<div class="card">';
				echo '<div class="header">Navigation Menu</div>';
				echo '<div class="content table-responsive">';
					...
					echo '<tbody>';
						foreach($cfg as $id => $navbarElement) {
							...
							echo '<tr>';
								echo '<td class="text-center" style="vertical-align:middle;"><a href="'.admincp_base('configuration/navmenu/delete/'.$id).'" rel="tooltip" title="" class="btn btn-danger btn-simple btn-xs" data-original-title="Delete"><i class="fa fa-times"></i></a></td>';
								...
								echo '<td class="text-center" style="vertical-align:middle;">';
									...
									echo '</label>';
								echo '</td>';
								echo '<td class="text-center" style="vertical-align:middle;"><button type="submit" name="navbar_submit" value="ok" class="btn btn-sm btn-primary">save</button></td>';
							echo '</tr>';
							echo '</form>';
						}

...
try {
	...
	if(check($_GET['delete'])) {
		try {
			...
			if(!check($_GET['delete'])) throw new Exception('Invalid id.');
	...
	if(check($_POST['usercp_submit'])) {
		try {
			# cfg
			$newCfg = loadConfig('usercp');
			if(!is_array($newCfg)) throw new Exception('Usercp configs empty.');
			
			if(!check($_POST['usercp_id'])) throw new Exception('Please fill all the form fields.');
			if(!check($_POST['usercp_type'])) throw new Exception('Please fill all the form fields.');
			if(!check($_POST['usercp_phrase'])) throw new Exception('Please fill all the form fields.');
			if(!check($_POST['usercp_link'])) throw new Exception('Please fill all the form fields.');
			...
			$newElementData = array(
				...
				'icon' => (check($_POST['usercp_icon']) ? $_POST['usercp_icon'] : 'usercp_default.png'),
...
echo '<div class="row">';
	echo '<div class="col-sm-12">';
		echo '<div class="card">';
			echo '<div class="header">User CP Menu</div>';
			echo '<div class="content table-responsive">';
				...
				echo '<tbody>';
					foreach($cfg as $id => $usercpElement) {
						...
						echo '<tr>';
							echo '<td class="text-center" style="vertical-align:middle;"><a href="'.admincp_base('configuration/usermenu/delete/'.$id).'" rel="tooltip" title="" class="btn btn-danger btn-simple btn-xs" data-original-title="Delete"><i class="fa fa-times"></i></a></td>';
							...
							echo '<td class="text-center" style="vertical-align:middle;">';
								...
								echo '</label>';
							echo '</td>';
							echo '<td class="text-center" style="vertical-align:middle;"><button type="submit" name="usercp_submit" value="ok" class="btn btn-sm btn-primary">save</button></td>';
						echo '</tr>';
						echo '</form>';
					}
					...
					echo '<tr>';
						...
						echo '<td class="text-center" style="vertical-align:middle;">';
							...
							echo '</label>';
						echo '</td>';
						echo '<td class="text-center" style="vertical-align:middle;"><button type="submit" name="new_submit" value="ok" class="btn btn-sm btn-primary">add</button></td>';
					echo '</tr>';
					echo '</form>';
				echo '</tbody>';
				echo '</table>';

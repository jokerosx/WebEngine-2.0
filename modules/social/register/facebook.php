...
// social status
if(!$socialCfg['enabled'] || !$socialCfg['provider']['facebook']['enabled']) throw new Exception(lang('error_87', array(Handler::websiteLink('register'))));

// adapter configuration
$adapterConfig = adapterConfig('facebook', 'social/register/facebook');
...
try {
	...
	if(check($_POST['webengineRegister_submit'])) {
		try {
			...
			if(!$AccountPreferences->linkFacebook()) throw new Exception(lang('error_81'));
	...
	echo '<div class="row">';
		...
		echo '</div>';
		echo '<div class="col-xs-6">';
			echo '<form class="form-horizontal" action="'.Handler::websiteLink('social/register/facebook').'" method="post">';

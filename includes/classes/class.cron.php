...
class Cron {
	...
	private $_checkConectionStatus = true;
	...
	public function addCron() {
		...
		$result = $this->we->query($query, $insertData);
	...
	public function executeCrons($showExecutionMessages=true) {
		...
		if($this->_checkConectionStatus) $this->_checkDatabaseConnection();
	...
	public function getCronList() {
		$result = $this->we->queryFetch("SELECT * FROM "._WE_CRON_." ORDER BY `cron_id` ASC");
	...
	/**
	 * clearAll
	 * clears the last run time of all cron jobs
	 */
	public function clearAll() {
		$this->we->query("UPDATE "._WE_CRON_." SET `cron_last_run` = NULL");
	}
	...
	private function _loadCronData() {
		...
		$result = $this->we->queryFetchSingle("SELECT * FROM "._WE_CRON_." WHERE `cron_id` = ?", array($this->_id));
		if(!is_array($result)) return;
		return $result;
	}
	...
	private function _cronExists($file) {
		$result = $this->we->queryFetchSingle("SELECT * FROM "._WE_CRON_." WHERE `cron_file` = ?", array($file));
	...
	private function _deleteCron() {
		if(!check($this->_id)) return;
		$result = $this->we->query("DELETE FROM "._WE_CRON_." WHERE `cron_id` = ?", array($this->_id));
	...
	private function _toggleStatus() {
		...
		$result = $this->we->query("UPDATE "._WE_CRON_." SET `cron_status` = ? WHERE `cron_id` = ?", array($newStatus, $this->_id));
		if($result) return true;
		return;
	}
	...
	private function _loadActiveCronData() {
		$result = $this->we->queryFetch("SELECT * FROM "._WE_CRON_." WHERE `cron_status` = 1 ORDER BY `cron_id` ASC");
		if(!is_array($result)) return;
		return $result;
	}
	...
	private function _updateCronLastRun($id) {
		if(!check($id)) return;
		$result = $this->we->query("UPDATE "._WE_CRON_." SET `cron_last_run` = CURRENT_TIMESTAMP WHERE `cron_id` = ?", array($id));
		if($result) return true;
		return;
	}
	...
	private function _checkDatabaseConnection() {
		...
		$MuOnline = Handler::loadDB('MuOnline');
		if($MuOnline->dead) {
			$offline = true;
		}
		
		$Me_MuOnline = Handler::loadDB('Me_MuOnline');
		if($Me_MuOnline->dead) {
			$offline = true;
		}
		
		if($offline) {
			if(!config('offline_mode')) setOfflineMode(true);
		} else {
			if(config('offline_mode')) setOfflineMode(false);
		}
	}

...
class Vote {
	...
	private $_configurationFile = 'account.vote';
	...
	function __construct() {
		...
		// credit configuration id
		$this->_creditConfig = $cfg['credit_config'];
		
		// vars
		$this->_topVotersStartDate = strtotime(date("Y-m-01 00:00"));
		$this->_topVotersEndDate = strtotime(date("Y-m-t 23:59"));
	...
	public function vote() {
		...
		// check credit config
		if($this->_creditConfig == 0) throw new Exception(lang('error_224'));
		...
		switch($configSettings['config_user_col_id']) {
			...
			case 'username':
				...
				break;
			case 'email':
				$creditSystem->setIdentifier($this->_email);
				break;
			default:
	...
	public function getTopVoters() {
		...
		foreach($result as $account) {
			...
			$return[] = array(
				'username' => $this->_accountInfo[_CLMN_USERNM_],
